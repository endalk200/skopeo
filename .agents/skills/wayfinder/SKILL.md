...
The map is a single issue on this repo's issue tracker, labelled with the canonical `wayfinder:map` role — the canonical artifact. Its tickets are child issues of the map. Use the repository's configured label mapping when the actual label string differs.
...
Each ticket is a **child issue** of the map; the tracker's issue id is its identity. Its body asks one sharply stated question that should fit within one fresh agent session without compaction or handoff. If it contains independent questions or requires a mid-session handoff, split it.
...
Routine tracker updates that directly record an agreed resolution can proceed without another checkpoint. If an update materially changes the destination, scope, or dependency structure, present the proposed change and get the user's approval before writing it.
