...
- **Local Markdown** → follow `docs/agents/issue-tracker.md`: write one file per ticket under `.scratch/<feature-slug>/issues/`, numbered in dependency order (blockers first). Each file's `Blocked by` line lists the numbers it depends on. Use the local ticket template below.
- **GitHub or GitLab** → publish one issue per ticket in dependency order (blockers first) so each ticket's blocking edges can reference real identifiers. Use the platform's native blocking / sub-issue relationship where it has one; otherwise set each ticket's "Blocked by" to the blocking issues. Apply the `ready-for-agent` triage label unless instructed otherwise — the tickets are agent-grabbable by construction.
- **Other trackers** → follow the custom publishing and dependency workflow recorded in `docs/agents/issue-tracker.md`.
...
A reference to the source spec file, if there is one. Otherwise omit this section.
